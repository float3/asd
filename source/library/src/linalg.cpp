...
Matrix::Matrix(const std::vector<double> &rowMajorData, size_t size1)
  ...
  linalghelper::runtime_check(size1_ * size2_ == rowMajorData.size(),
                              "Inconsistent sizes!");
}
...
Matrix::Matrix(const std::vector<Vector> &vectorOfRows)
  ...
  if (size1_ != 0) {
    ...
    for (size_t i = 0; i < size1_; ++i) {
      ...
      std::copy(vectorOfRows[i].begin(), vectorOfRows[i].end(),
                data_.begin() + (i * size2_));
    }
  }
}
...
void writeRow(const VectorType &vector, size_t size, std::ostream &out,
              size_t digits) {
...
void write(const Vector &vector, std::ostream &out) {
  linalghelper::writeRow([&](size_t i) { return vector[i]; }, vector.size(),
                         out, 12);
}
...
void write(const Matrix &matrix, std::ostream &out) {
  ...
  for (size_t i = 0; i < size1; ++i) {
    linalghelper::writeRow([&](size_t j) { return matrix(i, j); }, size2, out,
                           12);
  }
}
...
void updatePermutation(const Matrix &matrix, PermutationVector &permutation,
                       size_t index, double singularTolerance) {
  ...
  auto pivot =
      std::max_element(permutation.begin() + index, permutation.end(), compare);

  linalghelper::runtime_check(std::abs(matrix(*pivot, index)) >
                                  singularTolerance,
                              "Matrix is singular!");
...
Vector solve(const Matrix &matrix, const Vector &rhs) {
  ...
  linalghelper::runtime_check(matrix.size2() == n,
                              "Solve needs a square matrix!");
  linalghelper::runtime_check(rhs.size() == n,
  ...
  auto permuteMatrix = [&](size_t i, size_t j) -> double & {
    return tmpM(permute[i], j);
  };
  ...
  std::transform(permute.begin(), permute.end(), x.begin(),
                 [&](size_t i) { return tmpB[i]; });
