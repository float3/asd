...
TEST_CASE("norm") {
  ...
  Matrix matrix(
      {Vector{1.2, 3.3, 9.1}, Vector{2.9, 8.6, 2.6}, Vector{-4.8, 0.3, -2.1}});
...
TEST_CASE("solve_zero_pivot") {
  ...
  Vector expectedSolution{7.57541894507922, -4.311354527696106,
                          -2.8003180763255577, 3.456252253714161};
  ...
  for (size_t i = 0; i < 4; ++i) {
    CHECK(computedSolution[i] ==
          Approx(expectedSolution[i]).epsilon(tolerance));
  }
}
